...
export function Route_WriteView() {
  ...
  const [inputs, setinputs] = useState({
    ...
    start_time: '',
    count: '',
    station_numID: null, // null 값 빈배열, length : 0 으로 취급
    input_numID: '',
    input_station: '',
  });
  const {
  ...
  const fetchpost = async () => {
    try {
      ...
      await fetch('http://112.164.190.87:5000/api/route_write', {
        method: 'post',
        headers: {
  ...
  return (
    <div className="container">
      <div className="title_header">{numID}번 버스정보</div>
      ...
      <div className="title_header">( {numID} )번 노선정보</div>
      ...
      <div className="table4">
        <div className="row3">
          ...
          <div className="slt_right" onClick={() => clickadd()}>
            ＋
          </div>
        </div>
      </div>
